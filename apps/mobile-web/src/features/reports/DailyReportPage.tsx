...
export function DailyReportPage() {
  ...
  const loadDailySummary = async () => {
    ...
    try {
      ...
      const collectionLiters = collections.reduce((sum, c) => sum + Number(c.quantity), 0)
      const collectionAmount = collections.reduce((sum, c) => sum + Number(c.totalAmount), 0)

      const deliveredItems = deliveries.filter((d) => d.status === 'DELIVERED')
      const salesLiters = deliveredItems.reduce((sum, d) => sum + Number(d.quantity), 0)
      const salesAmount = deliveredItems.reduce((sum, d) => sum + Number(d.totalAmount), 0)
